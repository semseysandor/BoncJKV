...
Public Class Rules
  ...
  Private Function ApplyRulesDeath(data As Dictionary(Of String, String)) As Boolean
    ...
    If CheckRequired(key, data) Then
      ...
      Select Case data.Item(key)
        Case "asu_iszb_sze"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként az aortát és a szívkoszorú verőereket kifejezett mértékben érintő, súlyos, általános érelmeszesedést állapítottunk meg. A halál okaként az arteria coronariák súlyos szűkülete és a szívizom idült ischaemiás elfajulása következtében kialakult szívelégtelenséget jelöltük meg"
        Case "asu_regi_sze"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként az aortát és a szívkoszorú verőereket kifejezett mértékben érintő, súlyos, általános érelmeszesedést állapítottunk meg. A bal kamrában régi szívizomelhalást figyelhettünk meg. A halál okaként az arteria coronariák súlyos szűkülete és a szívizom idült ischaemiás elfajulása következtében kialakult szívelégtelenséget jelöltük meg"
        Case "asu_heveny"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként az aortát és a szívkoszorú verőereket kifejezett mértékben érintő, súlyos, általános érelmeszesedést állapítottunk meg. A nagyfokú arteria coronaria sclerosis és szűkület heveny első fali/hátsó fali szívizomelhalást eredményezett, melyet a halál okaként jelöltünk meg"
        Case "asu_heveny_tamp"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként az aortát és a szívkoszorú verőereket kifejezett mértékben érintő, súlyos, általános érelmeszesedést állapítottunk meg. A nagyfokú arteria coronaria sclerosis és szűkület heveny első fali/hátsó fali szívizomelhalást eredményezett, itt a szabad rupturált, következményes heveny szívburki vérgyülemet okozva. A halál okaként a szívtamponádot jelöltük meg"
        Case "copd"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként idült obstruktív tüdőbetegséget állapítottunk meg, illetve következményes idült tüdőeredetű szívbetegséget figyelhettünk meg, melyet a tágult túltengett jobb szívfél és a belszervi pangás morfológiailag alátámasztott. A halál okaként a szívelégtelenséget jelöltük meg"
        Case "emphy"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként súlyos tüdőtágulatot állapítottunk meg, illetve következményes idült tüdőeredetű szívbetegséget figyelhettünk meg, melyet a tágult túltengett jobb szívfél és a belszervi pangás morfológiailag alátámasztott. A halál okaként a szívelégtelenséget jelöltük meg"
        Case "ht_hszb_sze"
          Content.Item(key) += "anamnézisében magas vérnyomás betegség szerepel. A patológiai vizsgálata során magas vérnyomásos szívtúltengést állapítottunk meg, melyet a körkörösen/tágult súlyosan túltengett bal kamra morfológiailag alátámasztott. A halál okaként a szívelégtelenséget jelöltük meg"
        Case "htszb_sze"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként magas vérnyomásos szívtúltengést állapítottunk meg, melyet a körkörösen/tágult, súlyosan túltengett bal kamra morfológiailag alátámasztott. A halál okaként a szívelégtelenséget jelöltük meg"
        Case "asu_tudo"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként az aortát és a szívkoszorú verőereket kifejezett mértékben érintő, súlyos, általános érelmeszesedést állapítottunk meg. A halál közvetlen oka a pulmonalis főtörzs oszlásának lumenét teljesen elzáró, ún. lovagló thromboembolia / a tüdőverőerek elsőrendű ágainak masszív vérrögös elzáródása volt. Az embolia feltételezhetően bal/jobb alsóvégtagi mélyvénás rögösödésből származott"
        Case "htszb_tudo"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként hypertensiv szívbetegséget állapítottunk meg. A halál közvetlen oka a pulmonalis főtörzs oszlásának lumenét teljesen elzáró, ún. lovagló thromboembolia / a tüdőverőerek elsőrendű ágainak masszív vérrögös elzáródása volt. Az embolia feltételezhetően bal/jobb alsóvégtagi mélyvénás rögösödésből származott"
        Case "ht_apo"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként a bal/jobb agyféltekét/a kisagyat érintő roncsoló agyvérzést állapítottunk meg, szövődményként agyi nyomásfokozódás és a nyúltvelői légzési-keringési rendszer nyomás alá kerülése miatt következményes cardiorespiratoricus elégtelenség kialakulásával, melyek a halál közvetlen okaként megjelölt szívmegálláshoz vezettek"
        Case "etil"
          Content.Item(key) += ""
        Case "avmelyvenas_tudo"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként jobb/bal alsó végtagi mélyvénás rögösödést állapítottunk meg. A halál közvetlen oka a pulmonalis főtörzs oszlásának lumenét teljesen elzáró, ún. lovagló thromboembolia / a tüdőverőerek elsőrendű ágainak masszív vérrögös elzáródása volt"
        Case "covid_pneumonia"
          Content.Item(key) += "patológiai vizsgálata során alapbetegségként igazolt Covid-19 fertőzést állapítottunk meg. A halál okaként a vírusos tüdőgyulladást jelöltük meg"
      End Select
    ElseIf AbortOnMissing Then
      Return False
    End If
